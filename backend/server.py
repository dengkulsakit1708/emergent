...
app = FastAPI(title="PT Sonar Nusantara Utama API")
...
@api_router.get("/")
async def root():
    return {"message": "PT Sonar Nusantara Utama API"}
...
async def create_contact_inquiry(payload: ContactInquiryCreate):
    ...
    logger.info(f"New contact inquiry from {inquiry.email} - type: {inquiry.inquiry_type}")
    return inquiry
